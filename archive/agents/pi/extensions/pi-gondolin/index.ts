...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_shutdown", async (_event, ctx) => {
    if (!vm) return;
    if (ctx.hasUI) ctx.ui.setStatus("gondolin", ctx.ui.theme.fg("muted", "Gondolin: stopping"));
    try {
      await vm.close();
    } finally {
      vm = null;
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    localCwd = ctx.cwd;
    await ensureVm(ctx);
    const guestCwdLine = `Current working directory: ${GUEST_WORKSPACE} (Gondolin VM, mounted from host: ${localCwd})`;
    const modified = event.systemPrompt.replace(/^Current working directory: .*$/m, guestCwdLine);
