 ...
 *
 * Source:
 * https://github.com/mitsuhiko/agent-stuff/blob/e707652bc7a1a55a42c983cd78803cc2feb38be4/pi-extensions/session-breakdown.ts
 *
 * /session-breakdown
 *
...
function weeksForRange(range: RangeAgg): number {
  ...
  const totalGridDays =
    Math.round((gridEnd.getTime() - gridStart.getTime()) / (24 * 3600 * 1000)) + 1;
...
async function computeBreakdown(sessionRoot: string, signal?: AbortSignal): Promise<BreakdownData> {
  const now = new Date();
  ...
  return { generatedAt: now, sessionRoot, ranges, palette };
}
  ...
  pi.registerCommand("session-breakdown", {
    ...
    handler: async (_args, ctx: ExtensionContext) => {
      if (!ctx.hasUI) {
        ...
        const data = await computeBreakdown(getSessionRoot(), undefined);
        const range = data.ranges.get(30)!;
        pi.sendMessage(
          {
            customType: "session-breakdown",
            content: [
              "Session breakdown (non-interactive)",
              `Sessions directory: ${data.sessionRoot}`,
              rangeSummary(range, 30),
              "",
              ...renderModelTable(range, 8),
            ].join("\n"),
            display: true,
          },
          { triggerTurn: false },
      ...
      const data = await ctx.ui.custom<BreakdownData | null>((tui, theme, _kb, done) => {
        const loader = new BorderedLoader(
          tui,
          theme,
          `Analyzing sessions in ${sessionRoot} (last 90 days)…`,
        );
        loader.onAbort = () => done(null);

        computeBreakdown(sessionRoot, loader.signal)
          .then((d) => done(d))
          .catch(() => done(null));
